"""
SaaSShorts：面向 SaaS 产品的 AI UGC 视频生成模块。

可根据 SaaS 链接生成更易传播的 TikTok/Instagram Reels 内容。
处理流程：
  1. 抓取并分析 SaaS 网站（Gemini）
  2. 生成视频脚本（Hook → 痛点 → 方案 → CTA）
  3. 生成 AI 演员人像（fal.ai / Flux Pro）
  4. 生成配音（ElevenLabs TTS）
  5. 生成口播视频（fal.ai / Kling Avatar v2）
  6. 生成 B-roll 片段（fal.ai）
  7. FFmpeg 合成成片并叠加字幕
"""
...
def research_saas_online(url: str, gemini_key: str) -> dict:
    """
    使用 Gemini + Google Search grounding 深度调研 SaaS 产品：
    覆盖测评站、Reddit、Twitter、竞品对比、价格反馈、用户口碑等信息源。
    """
...
def _fal_run(model_id: str, input_data: dict, fal_key: str, timeout: int = 600) -> dict:
    """
    向 fal.ai 队列提交任务并轮询结果，完成后返回输出。
    优先使用 submit 响应中返回的状态/结果 URL（符合官方建议）。
    """
    headers = {
    ...
    if not request_id:
        # 无 request_id 时视为同步返回
        return submit_data

    # 优先使用 submit 响应内提供的 URL（最可靠）
...
def generate_talking_head(
    ...
    """使用 fal.ai 的 Kling Avatar v2 Standard 生成口播视频。"""
    print(f"[SaaSShorts] 🗣️ Generating talking head (Kling Avatar v2)...")

    # 先将图片与音频上传到 fal.ai CDN
...
def generate_talking_head_lowcost(
    ...
    """
    低成本口播方案：Hailuo 2.3 Fast 图生视频 → VEED Lipsync。
    成本显著低于 Kling Avatar v2。
    """
    print(f"[SaaSShorts] 🗣️ Generating talking head (Low Cost: Hailuo + VEED Lipsync)...")

    # 第 1 步：用 Hailuo 2.3 Fast 先生成 6s 视频，并做缓存避免重试重复生成
...
def generate_broll(
    ...
    """生成 B-roll：先生成静态图，再用 FFmpeg 做 Ken Burns 动效。"""
    ...
    # Flux 2 Pro 结果可能位于 images 或 output 字段
    images = result.get("images") or result.get("output", [])
    if not images:
    ...
    # 第 2 步：Ken Burns 缓慢推近 + 轻微平移
    fps = 30
    total_frames = dur_secs * fps
    # 在整段时长内从 1.0x 缓慢推到 1.15x（更电影感）
    zoompan_filter = (
        f"scale=2160:3840,"
    ...
    cmd = [
        "ffmpeg", "-y",
        "-loop", "1", "-i", img_path,          # 输入 0：图片
        "-f", "lavfi", "-i", "anullsrc=r=44100:cl=stereo",  # 输入 1：静音音轨
        "-vf", zoompan_filter,
        "-t", str(dur_secs),
        "-map", "0:v", "-map", "1:a",
...
def _get_media_duration(path: str) -> float:
    ...
    return 30.0  # 失败时回退到 30 秒估算


def _format_ass_time(seconds: float) -> str:
    """将秒数格式化为 ASS 字幕时间格式：H:MM:SS.cc"""
    h = int(seconds // 3600)
...
def generate_tiktok_subs(audio_path: str, output_path: str, max_words: int = 3) -> str:
    """
    基于 Whisper 转写结果生成 TikTok 风格 ASS 字幕。

    样式：大号加粗居中、每次 1-3 词、白字黑描边，并尽量贴合真实语音时间戳。
    """
...
def composite_video(
    ...
    if srt_path.endswith(".ass"):
        # ASS 自带样式，直接用 ass filter
        sub_filter = f"ass='{safe_sub}'"
    else:
        # SRT 兜底：强制注入近似 TikTok 样式
        sub_style = (
...
def generate_full_video(
    ...
    """
    SaaSShorts 全流程视频生成编排。

    参数：
        script: generate_scripts() 返回的单条脚本
        config: {
            "fal_key": str,
            "elevenlabs_key": str,
            "voice_id": str (可选),
            "actor_description": str (可选，覆盖脚本值),
        }
        output_dir: 输出目录
        log: 进度日志回调
