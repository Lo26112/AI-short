...
class VideoEditor:
    ...
    def get_ffmpeg_filter(self, video_file_obj, duration, fps=30, width=None, height=None, transcript=None):
        """向 Gemini 请求原始 FFmpeg filter 字符串。"""
        if width is None or height is None:
            # 调用方未传尺寸时提供兜底值，保证提示词可用
    ...
    def _enforce_zoompan_output_size(cls, filter_string: str, width: int, height: int) -> str:
        """强制 zoompan 输出尺寸与输入视频一致。"""
        parts = cls._split_filter_chain(filter_string)
        out_parts: list[str] = []
        for part in parts:
            if "zoompan=" in part:
                # 在 zoompan 参数内强制写入 s=宽x高
    ...
    def _sanitize_filter_string(filter_string: str) -> str:
        """
        对 Gemini 生成的表达式做尽力清洗：
        将 t<3、on>=75 等比较运算转换为 FFmpeg 函数形式（lt/gte 等），
        以提升不同 FFmpeg 版本下的解析稳定性。
        """
        s = filter_string
    ...
    def apply_edits(self, input_path, output_path, filter_data):
        ...
        # 显式使用 UTF-8 环境，减少子进程编码报错
        env = os.environ.copy()
        # 在精简容器中优先使用 C.UTF-8 以保证参数编码可用
        env["LANG"] = "C.UTF-8"
        env["LC_ALL"] = "C.UTF-8"
        
        try:
            # 当文件系统编码异常时，转为 bytes 传参可降低 UnicodeEncodeError 风险
            cmd_bytes = []
            for arg in cmd:
